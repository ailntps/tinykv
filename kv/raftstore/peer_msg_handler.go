...
func (d *peerMsgHandler) processAdminRequest(entry *eraftpb.Entry, msg *raft_cmdpb.RaftCmdRequest, wb *engine_util.WriteBatch) {
	...
	case raft_cmdpb.AdminCmdType_Split:
		lastindex, _ := d.peerStorage.LastIndex()
		d.LastCommittedSplitIndex = lastindex
...
func (d *peerMsgHandler) processConfChange(entry *eraftpb.Entry, confChange *eraftpb.ConfChange, wb *engine_util.WriteBatch) {
	...
	err := raftReq.Unmarshal(confChange.Context)
	if nil != err {
		panic(err)
	}
	//update region information regionEpoch and peer
	region := d.Region()
	//check this update wheater come from a same region,
	//this can use raftReq.header.RegionEpoch.Conf_ver check
	//util have a function CheckRegionEpoch which check above
	err = util.CheckRegionEpoch(raftReq, region, true)
	if errEpochNotMatching, ok := err.(*util.ErrEpochNotMatch); ok {
		d.handleProposal(entry, func(p *proposal) {
			p.cb.Done(ErrResp(errEpochNotMatching))
		})
	...
	case eraftpb.ConfChangeType_AddNode:
		n := searchRegionPeer(region, confChange.NodeId)
		if n == len(region.Peers) {
			//add peer and set ConfVer++
			peer := raftReq.AdminRequest.ChangePeer.Peer
			region.Peers = append(region.Peers, peer)
			region.RegionEpoch.ConfVer++
			//write region when region update
			meta.WriteRegionState(wb, region, rspb.PeerState_Normal)
			//storeMeta set Meta data in store
			storeMeta := d.ctx.storeMeta
			storeMeta.Lock()
...
func (d *peerMsgHandler) proposeAdminRequest(msg *raft_cmdpb.RaftCmdRequest, cb *message.Callback) {
	...
	case raft_cmdpb.AdminCmdType_ChangePeer:
		//confChange need tell client,so need cb.
		if d.RaftGroup.Raft.PendingConfIndex > d.peerStorage.AppliedIndex() {
			return
		}
		confChangeReq := msg.AdminRequest.ChangePeer
		data, err := msg.Marshal()
		if err != nil {
			panic(err)
		}
		confChange := eraftpb.ConfChange{ChangeType: confChangeReq.ChangeType, NodeId: confChangeReq.Peer.Id, Context: data}
		p := &proposal{index: d.nextProposalIndex(), term: d.Term(), cb: cb}
		d.proposals = append(d.proposals, p)
		d.RaftGroup.ProposeConfChange(confChange)

	case raft_cmdpb.AdminCmdType_CompactLog:
		//compactLog dont need tell client
		data, err := msg.Marshal()
		if err != nil {
			panic(err)
		}
...
//ScheduleCompactLog used to put a raftLogGCTask in raftLogGCTaskSender
